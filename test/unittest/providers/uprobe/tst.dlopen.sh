...
{
printf("Clause 3");
	printf("expected %d, bad %d, returns %d, entries %d\n", self->expected, self->bad, self->returns, self->entries);
}

uprobe:libswlazy.so:lazy_probe:return
/self->expected && self->returns == 1/
{
printf("Clause 4");
	self->expected = 0;
	exit(self->bad != 0 || self->entries != 1);
}
